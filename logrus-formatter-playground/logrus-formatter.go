...
type TextFormatter struct {
	...
	// Pad msg field with spaces on the right, making it fixed width / left aligned
...
func (f *TextFormatter) printColored(b *bytes.Buffer, entry *logrus.Entry, keys []string, timestampFormat string, colorScheme *compiledColorScheme) {
	...
	level := fmt.Sprintf("[%s]", levelText)
	level = fmt.Sprintf("%-7s", level) // left justify and pad with spaces on the right
	level = levelColor(level)          // add colour
	prefix := ""
